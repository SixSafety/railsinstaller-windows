module RailsInstaller
    ...
    Sqlite3.files.each do |file|
      if File.exist?(File.join(Stage, file))
        FileUtils.mv(
          ...
          File.join(Stage, Ruby231.rename, "bin", file)
        )
      end
    end
  end
  ...
  def self.link_devkit_with_ruby
    ...
    ruby_path = File.join(Stage, Ruby231.rename)
  ...
  def self.stage_gems
    section Gems
    build_gems(File.join(Stage, Ruby231.rename), Gems.list)
    build_gem(File.join(Stage, Ruby231.rename), "pg", {
      :args => [
          "--",
  ...
  def self.stage_todo_application
    ...
    gem_install File.join(Stage, Ruby231.rename), "bundler", :version => "1.15.3"

    ruby_binary("bundle", "install", "", File.join(Stage, Ruby231.rename), File.join(applications_path, "todo"))
  end
  ...
  def self.stage_rails_sample_application
    ...
    ruby_binary("rails", "new", "sample", File.join(Stage, Ruby231.rename))
  end
